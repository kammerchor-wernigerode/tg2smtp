...
class VoiceDownloaderTests {
    ...
    private @Mock Voice voice;

    @BeforeEach
    void setUp() {
        downloader = new VoiceDownloader(properties, executor);
    }

    @Test
    void initializeNullArguments_shouldThrowException() {
        assertThrows(IllegalArgumentException.class, () -> new VoiceDownloader(null, executor));
        assertThrows(IllegalArgumentException.class, () -> new VoiceDownloader(properties, null));
    }

    @Test
    @SneakyThrows
    void failingExecution_shouldThrowException() {
        when(voice.getFileId()).thenReturn("foo");
        when(executor.apply(any())).thenThrow(TelegramApiException.class);

        assertThrows(IOException.class, () -> downloader.download(voice));
    }
